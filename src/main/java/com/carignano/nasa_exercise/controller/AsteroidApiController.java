...
public class AsteroidApiController {
    ...
    public ResponseEntity<NasaAwcResponse<List<AsteroidPath>>> getAsteroidPaths(@PathVariable @Positive@NotNull Long asteroidId, @RequestParam(required = false) @DateTimeFormat(pattern = ProjectConstants.DateFormat.STANDARD_DATE_FORMAT) LocalDate fromDate, @RequestParam(required = false) @DateTimeFormat(pattern = ProjectConstants.DateFormat.STANDARD_DATE_FORMAT) LocalDate toDate){
